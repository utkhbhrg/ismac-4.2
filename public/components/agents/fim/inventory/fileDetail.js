    ...
    renderFileDetailsPermissions(value) {
        if (((this.props.agent || {}).os || {}).platform === 'windows' && value && value !== '-') {
            const components = value.split(', ').map(userNameAndPermissionsFullString => {
                ...
                return (react_1.default.createElement(eui_1.EuiToolTip, { key: `permissions-windows-user-${username}`, content: permissions.join(', '), title: `${username} permissions` },
                    react_1.default.createElement(eui_1.EuiBadge, { color: 'hollow', title: null, style: { margin: '2px 2px' } }, username)));
            });
            return react_1.default.createElement(util_1.TruncateHorizontalComponents, { components: components, labelButtonHideComponents: (count) => `+${count} users`, buttonProps: { size: 'xs' }, componentsWidthPercentage: 0.85 });
        }
        ;
        return value;
    }
