...
export class ApiIsDown extends Component {
      ...
      for (let idx in entries) {
        ...
        try {
          ...
          const downReason = typeof error === 'string' ? error :
            (error || {}).message || ((error || {}).data || {}).message || 'ISMAC is not reachable';
  ...
  render() {
    const apiExample = `# Example ISMAC API configuration
hosts:
    - production:
...
`;
    ...
    const checkConnectionChildren = (
      <div>
        <EuiText>
          Check that the ISMAC server can reach the configured API(s).
        </EuiText>
        <EuiSpacer />
        <EuiButton
        ...
        <EuiSpacer />
        <EuiText>Already configured API(s)</EuiText>
        <EuiSpacer />
        {(!this.state.error && (
          <EuiBasicTable
    ...
    const steps = [
      {
        title: 'Check the API service status',
        children: (
          <div>
